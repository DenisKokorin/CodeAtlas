...
* редактировать сгенерированную документацию через визуальный Markdown-редактор;
...
После генерации документации пользователь может открыть любую ревизию документации и нажать кнопку `Редактировать документацию`.

Редактирование выполняется через Markdown-редактор на frontend. Пользователь может использовать панель инструментов для заголовков, жирного/курсивного текста, списков, ссылок, таблиц, блоков кода и переключения в source mode.

Backend сохраняет изменения в выбранную ревизию документации без создания новой ревизии:
...
После ручного сохранения у версии документации меняется источник:

```text
documentation_source = manual_edit
```

Для исходно сгенерированной документации используется:

```text
documentation_source = generated
```

Frontend использует это поле для отображения бейджа `Сгенерировано ИИ` или `Отредактировано вручную`. Business Summary, Critical Parts и Quality Assessment при ручном редактировании не изменяются. Мультиформатный экспорт работает уже с обновлённым Markdown-текстом документации.
...
Frontend адаптирован под актуальный backend CodeAtlas:

* регистрация и авторизация через JWT;
* добавление, просмотр, редактирование и удаление репозиториев;
* генерация документации с обязательным указанием `app_version`;
* просмотр актуальной документации;
* ручное редактирование документации через Markdown-редактор;
* просмотр истории версий и ревизий документации;
* просмотр Business Summary отдельным текстовым блоком;
* просмотр Critical Parts отдельным текстовым блоком;
* dashboard для Quality Assessment;
* экспорт документации в `markdown`, `txt`, `html`, `docx` и `json`;
* поддержка светлой и тёмной темы.

Мультиформатный экспорт применяется только к основной технической документации. Business Summary, Critical Parts и Quality Assessment отображаются отдельно и не смешиваются с Markdown-документацией.
